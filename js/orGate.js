function logicImplementation1(vccRow,vccColumn,gndRow,gndColumn,startCol){
    if(arrayBody[vccRow][vccColumn] == 1 && arrayBody[gndRow][gndColumn] == 0){
        let count = 0;
        while(count != 2){
            if(arrayBody[4][startCol]==0 && arrayBody[4][startCol+1]==0){
                for(let i=4;i<8;i++){
...
function orGateLogic(place){
    ...
    if(place=="place1"){
        ...
        let startCol = 5;
        logicImplementation1(vccRow,vccColumn,gndRow,gndColumn,startCol);
    }
